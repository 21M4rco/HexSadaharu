...
`tools/make_assets.py` and `tools/sculpt_coat.py` author the welded coat, skin weights, rig and item pixel art; `tools/render_model.py` produces a depth-rendered contact sheet from the exact rig. These optional tools need Python, Pillow and NumPy. They do not compile Java.
